...
describe('Quest', () => {
  ...
  it('can validate truth tables', () => {
    ...
    quest.engine.parse('K(K(KI))').expect(never.input[0] as Expr);
    (never.input[0] as Expr).expect(never.expr as Expr);
  });

  it('rejects bad specs', () => {
    expect(() => new Quest({ input: 'foo', cases: [['f->f']] as unknown as TestCase[] })).to.throw(/exactly 2/);
    expect(() => new Quest({ input: 'foo', cases: [['f', 'I', 'I']] as unknown as TestCase[] })).to.throw(/exactly 2/);
  });
  ...
  it('honors own restrictions', () => {
    const ski = new SKI();
    const quest = new Quest({ engine: ski, env: ['foo'], input: 'f', cases: [['f x', 'x']], allow: 'SK' });
  ...
  it('allows named constants in input', () => {
    ...
    // TODO wtf? alternates between null & undefined
    expect(passing.reason ?? null).to.equal(null);
  ...
  it('can self-check metadata', () => {
    ...
    const result = quest.verify({ seen, date: true }) as Record<string, unknown>;
    expect(result).to.be.an('object');
  ...
  it('can self-check but in groups', () => {
    ...
    const contentArr = content as unknown[];
    expect(contentArr).to.be.an('array').with.lengthOf(1);
    expect(contentArr[0]).to.be.an('object');
    expect((contentArr[0] as { intro: string }).intro).to.match(/unclosed.*\bp\b/i);
  });
