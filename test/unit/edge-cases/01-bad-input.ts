...
import { RefinedFormatOptions } from '../../../src/expr';
...
describe('SKI', () => {
  ...
  it('Forbids strange stuff in named terms', () => {
    expect(() => new Alias({ foo: 42 } as unknown as string, SKI.S)).to.throw(/improper name/);
  });
  ...
  it('forbids adding garbage to known terms', () => {
    const ski = new SKI();
    expect(() => ski.add('foobar', { foo: 42 } as unknown as string)).to.throw(/Expr\b.*\bstring/);
  });
});

class Noob extends Expr {
  formatImpl (options: RefinedFormatOptions, nargs: number): string {
    throw new Error('Noob cannot be stringified');
  }
}
