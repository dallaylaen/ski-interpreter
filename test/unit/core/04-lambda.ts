...
describe('Lambda', function () {
  ...
  it('works with aliases', () => {
    const ski = new SKI();
    ski.add('T', 'S(K(SI))K');
    const expr = new Lambda(x, ski.parse('T'));
  ...
  it('forbids empty var list', () => {
    expect(() => new Lambda([] as unknown as FreeVar, x)).to.throw(/Lambda argument must be a FreeVar/i);
  });
  it('forbids duplicate vars', () => {
    const x1 = new SKI.classes.FreeVar('x');
    const x2 = new SKI.classes.FreeVar('x');
    expect(() => new Lambda([x1, x2] as unknown as FreeVar, x1)).to.throw(/Lambda argument must be a FreeVar/i);
  });
