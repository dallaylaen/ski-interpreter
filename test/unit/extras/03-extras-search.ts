...
import { Expr, TermInfo } from '../../../src/expr';
import { SearchCallback, SearchOptions, SearchProgress } from '../../../src/extras';
 ...
 */
function runSearch<T> (seed: Expr[], options: SearchOptions, predicate: SearchCallback<T>) {
  const gen = search(seed, options, predicate);
  let last: SearchProgress<T> | undefined;
  const found: Expr[] = [];
...
describe('SKI.extras.search', () => {
  ...
  describe('some basic conditions', () => {
    it('generates all possible combinations without repetition', () => {
      ...
      const gen = SKI.extras.search<boolean>([x, y], { depth: 5, infer: false }, () => 0);
      let last: SearchProgress<boolean> | undefined;
    ...
    it('honors maxSize option', () => {
      const gen = SKI.extras.search([SKI.K], { depth: 20, maxSize: 3 }, (_: Expr, p: TermInfo) => (p.normal ? { found: true, offset: 0 } : -1));
      for (const p of gen) {
  ...
  check('finds S', [SKI.S], {}, (e: Expr) => e === SKI.S ? { found: true, stop: true } : 0);
  check('finds I', [SKI.S, SKI.K], {}, (e: Expr) => getsto(e, [x], x) ? { found: true, stop: true } : 0);
  ...
  check('finds B', [SKI.S, SKI.K], {},
    (e: Expr) => getsto(e, [x, y, z], x.apply(y.apply(z))) ? { found: true, stop: true } : 0);
  ...
  describe('new semantics', () => {
    it('found and stop are orthogonal: found without stop keeps searching', () => {
      const found: (Expr | undefined)[] = [];
      for (const p of SKI.extras.search([SKI.S, SKI.K], { tries: 500 },
        (e: Expr) => e === SKI.S || e === SKI.K ? { found: true } : 0
      ))
...
function check<T> (
  ...
  predicate: SearchCallback<T>,
  extras?: (res: ReturnType<typeof runSearch<T>>, trace: { e: Expr, props: TermInfo }[]) => void
) {
  describe(name, () => {
    const trace: { e: Expr, props: TermInfo }[] = [];
    const wrap: SearchCallback<T> = (e, props) => {
      trace.push({ e, props });
...
function getsto (expr: Expr, args: Expr[], exp: Expr): boolean {
