...
describe('SKI.*', () => {
  ...
  it('provides Church numbers', done => {
    ...
    expect(SKI.church('0' as unknown as number).apply(x, y).run().expr.format({ terse: false })).to.equal('y');
    expect(SKI.church('1' as unknown as number).apply(x, y).run().expr.format({ terse: false })).to.equal('x(y)');
    expect(
      SKI.church('5' as unknown as number).apply(x, y).run().expr.format({ terse: false })
    ).to.equal('x(x(x(x(x(y)))))');

    // invalid numbers
    expect(() => SKI.church(-1)).to.throw(/must be a non-?negative/);
    expect(() => SKI.church('foo' as unknown as number)).to.throw(/must be a non-?negative/);
