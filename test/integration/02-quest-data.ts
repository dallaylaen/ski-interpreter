...
const uniqQuest   = new Set<string>();
const uniqChapter = new Set<string>();

let questSolutions: Record<string, unknown> = {};
  ...
  it('has index', async () => {
    ...
    describe('index', () => {
      for (let n = 0; n < index.chapters.length; n++) {
        const entry = typeof index.chapters[n] === 'string'
          ? { link: index.chapters[n] as string }
          : index.chapters[n] as Record<string, unknown> & { link: string };
...
function verifyChapter (entry: Record<string, unknown> & { link?: string; id?: string }, n: number): void {
  describe('chapter ' + n + ' ' + (entry.link ?? 'empty'), () => {
    ...
    it('passes group verify() checks', () => {
      ...
      const contentErrors = (findings.content as unknown[] | undefined)?.filter(item => item !== null);
    ...
    for (let i = 0; i < chapter.content!.length; i++) {
      ...
      describe('quest ' + entry.link + ' [' + i + '] ' + q.id, () => {
        ...
        it('has date', () => {
          ...
          expect(new Date(date as string)).to.be.instanceof(Date);
          expect(new Date(date as string)).to.be.within(new Date('2024-07-15'), new Date());
        });

        it('has unique id', () => {
          const id = q.meta?.id as string;
        ...
        it('passes quest verify() checks', () => {
          const findings = q.verify({
            date:      true,
            solutions: questSolutions as Parameters<typeof q.verify>[0]['solutions'],
          });
        ...
        it('passes passing and fails failing solutions, if given', () => {
          const nope = q.verifySolutions(questSolutions as Parameters<typeof q.verifySolutions>[0]);
          if (nope !== null) {
