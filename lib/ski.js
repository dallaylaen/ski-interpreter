...
class Ast {
   ...
   * TODO: return null on no changes, just like step() does, to save memory.
...
class App extends Ast {
  ...
  subst (...list) {
    return this.fun.subst(...list).apply(...this.args.map(t => t.subst(...list)));
  }
...
class FreeVar extends Named {
  subst (...list) {
    for (const [x, val] of list)
      if (this === x) return val;
    return this;
  }
}
...
class Lambda extends Ast {
  ...
  constructor (args, impl) {
    super();
    if (args.length === 0)
      throw new Error('empty alr list in lambda');
    const known = {};
    for (const x of args) {
      if (known[x]++)
        throw new Error('Duplicate free var ' + x + ' in lambda expression');
    }
  ...
  reduce (input) {
    ...
    for (let i = 0; i < this.args.length; i++)
      zip.push([this.args[i], input.shift()]);

    return this.impl.subst(...zip).apply(...input);
  }
