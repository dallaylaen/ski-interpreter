 ...
 */
const effects = {};
class Effect extends Expr {
  /**
   * @param {function (e: Expr): expr} fun
   */
...
/**
 *
 * @type {{[key: string]: (function(Expr): Expr)}}
 */

new Effect('number', expr => {
  const zero = new Church(0);
  const inc = new Native('inc', 1, (next, ...tail) => {
    if (!(next instanceof Church))
      throw new Error('Attempt to cast a random expression to Nat');
    return new Church(next.n + 1);
  });
  const outcome = expr.run({ max: 1000000 }, inc, zero).expr;
  if (outcome instanceof Church)
    return outcome;
  throw new Error('Unexpected expression in numeric calculation: ' + outcome);
}).register();
...
const combChars = new Tokenizer('[()]', '[A-Z]', '[a-z_][a-z_0-9]*', '\\b[0-9]+\\b', '->', '![a-z]+\\b');

console.log(combChars);
  ...
  parseLine (source, vars = {}, options = {}) {
    ...
    for (const c of tokens) {
      ...
      if (c === '(')
        ...
        const eff = effects[c];
        if (eff === null)
          throw new Error('Unknown effect ' + c);
        stack.push(eff.apply(stack.pop()));
      } else {
        const f = stack.pop();
