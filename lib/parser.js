...
class SKI {
   ...
   * @param {{[keys: string]: Expr}} [options.vars]
  ...
  parseLine (source, vars = {}, options = {}) {
    ...
    /** @type {Expr[]} */
    const stack = [empty];
    const context = options.context || SKI; // default is global unbound vars
    ...
    for (const c of tokens) {
      ...
      if (c === '(')
        ...
        const x = vars[c] ?? this.known[c] ?? (vars[c] = new FreeVar(c, context));
        stack.push(f.apply(x));
      }
    }
