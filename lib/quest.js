...
class Quest {
  ...
  add (opt, ...terms) {
    ...
    this.cases.push(new Case(this.input, terms, opt, this.varsFull));
    return this;
  }
...
class Case {
   ...
   * }} options
   */
  constructor (input, terms, options = {}) {
    if (terms.length !== 2)
    ...
    const vars = { ...(options.vars ?? {}) }; // shallow copy to avoid modifying the original

    [this.e1, this.e2] = terms.map(src => new Lambda(input, options.engine.parse(src, vars)));
  }
