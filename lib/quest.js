...
class Quest {
   ...
   */
  constructor (options = {}) {
    const { allow, numbers, vars, cases, lambdas, subst, engine, engineFull, ...meta } = options;
    ...
    this.vars = {};
    this.subst = Array.isArray(subst) ? subst : [subst ?? 'f'];
  ...
  check (...input) {
    try {
      const prepared = input
        ...
        .map((expr, i) => this.subst[i] ? new Alias(this.subst[i], expr) : expr);
