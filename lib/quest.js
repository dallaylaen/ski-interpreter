...
class Case {
  ...
  constructor (terms, options = {}, vars = {}, engine = new SKI()) {
    const name = terms.length > 2 ? terms.shift() : undefined;
    ...
    if (name) {
      const inputAlias = new FreeVar(name);
      vars[name] = inputAlias;

      this.e1 = new Lambda([inputAlias], prepare(e1));
      this.e2 = new Lambda([inputAlias], prepare(e2));
    } else {
      this.e1 = prepare(e1);
      this.e2 = prepare(e2);
    }
