...
class App extends Expr {
  ...
  rewriteSKI () {
    return this.fun.rewriteSKI().apply(...this.args.map(x => x.rewriteSKI()));
  }
...
// S(S(KS)K)
const SB = native.S.apply(native.S.apply(native.K.apply(native.S), native.K));
...
class Lambda extends Expr {
  ...
  rewriteSKI () {
    const impl = this.impl.rewriteSKI();
    if (impl === this.arg)
      return native.I;
    if (!impl.getSymbols().has(this.arg))
      return native.K.apply(impl);
    if (impl instanceof App) {
      const [fun, arg] = impl.split();
      // try eta reduction
      if (arg === this.arg && !fun.getSymbols().has(this.arg))
        return fun.rewriteSKI();
      // fall back to S
      return native.S.apply((new Lambda(this.arg, fun)).rewriteSKI(), (new Lambda(this.arg, arg)).rewriteSKI());
    }
...
class Church extends Native {
  ...
  rewriteSKI () {
    return this.reduce([SB, native.K.apply(native.I)]);
  }
