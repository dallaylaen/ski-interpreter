...
class Expr {
  ...
  canonize (options = {}) {
    ...
    // push arguments into the expression until it either hangs,
    // becomes terminal, or we run out of arguments (in which case reset).
    let steps = 0;
    let found = false;
    let expr = this;
    let jar = [];
    for (let i = 0; ; i++) {
      ...
      // if expr is a lambda, substitute argument by hand,
      // otherwise a nonterminating expr + app = hang
      expr = calc.expr instanceof Lambda
        ? calc.expr.impl.subst(calc.expr.arg, next) ?? calc.expr.impl
        : calc.expr.apply(next);
    }
    ...
    let proper = false;
    let duplicate = !!dup.size;
    let discard = !!skip.size;
    if (expr.freeOnly())
      proper = isSubset(symbols.keys(), new Set(jar));
    else {
      // could not make any sense by adding arguments to this
      if (!(expr instanceof App))
        throw new Error('Failed to canonize expression: ' + this);
      // in theory can run away if expr is looped

      const [fun, arg] = expr.split().map(x => x.canonize({ ...options, maxArgs: maxArgs - 1 }));
      expr = fun.canonical.apply(arg.canonical);
    ...
    return {
      ...
      proper,
      linear:    proper && !duplicate && !discard,
      discard,
      duplicate,
      steps,
      ...(skip.size ? { skip } : {}),
...
class Alias extends Named {
  ...
  canonize (options = {}) {
    return this.expand().canonize(options);
  }
