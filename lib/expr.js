...
class Expr {
  ...
  * lambdify (options = {}) {
    ...
    const varGen = options.varGen ?? freeSeq({ prefix: 'x', html: options.html });
    const max = options.max ?? globalOptions.max;
    const maxArgs = options.maxArgs ?? globalOptions.maxArgs;
    let steps = 0;
    const jar = [];
    ...
    for (let i = 0; i < maxArgs; i++) {
      const next = expr.run({ max });
      steps += next.steps;
      if (!next.final) {
        yield { expr: maybeLambda(jar, expr), steps, final: false };
        break;
      }
      expr = next.expr;
      if (!expr.wantsArgs())
    ...
    // try to short circuit if nothing but lambdas & free vars are left
 ...
 * @param {number} options.latinBefore
 *
 */
function freeSeq (options = {}) {
  ...
  const latinBefore = options.latinBefore ?? 0;
