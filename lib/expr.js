...
class App extends Expr {
  ...
  ski () {
    return this.fun.ski().apply(...this.args.map(x => x.ski()));
  }
...
const I = new Native('I', 1, x => x);
const K = new Native('K', 2, x => y => x);
const S = new Native('S', 3, x => y => z => x.apply(z).apply(y.apply(z)));
// S(S(KS)K)
const SB = S.apply(S.apply(K.apply(S), K));
  ...
  ski () {
    ...
    if (impl instanceof App) {
      ...
      if (arg === this.arg && !fun.getSymbols().has(this.arg))
        return fun.ski();
      // fall back to S
      return S.apply((new Lambda(this.arg, fun)).ski(), (new Lambda(this.arg, arg)).ski());
    }
...
class Church extends Native {
  ...
  ski () {
    return this.reduce([SB, K.apply(I)]);
  }
