...
/**
 *   @description for App.toString().
 *   Can lump terms together, if
 *   term types: 0 - unknown, 1 - in parens, 2 - uppercase 1letter, 3 - lowercase ids, 4 - numbers
 *   has(left *8  + right)
 */
  ...
  toString (opt = {}) {
    if (opt.terse) {
      ...
      for (const term of [this.fun, ...this.args]) {
        ...
        if (!canLump.has(oldType | newType) && out.length > 0)
          out.push(' ');
        out.push(s);
        oldType = newType << 3;
      }
