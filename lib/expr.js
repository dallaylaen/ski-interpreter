...
class Expr {
   ...
   * }}
   */
  guess (options = {}, preArgs = [], steps = 0) {
    const maxArgs = options.maxArgs ?? globalOptions.maxArgs;
    if (preArgs.length > maxArgs)
    ...
    const next = this.run({ max: options.max ?? globalOptions.max });
    steps += next.steps;
    if (!next.final)
      return { grounded: false, steps };
    ...
    if (next.steps !== 0)
      return next.expr.guess(options, preArgs, steps);

    if (this._firstVar())
      return { grounded: false, steps };

    const push = nthvar(preArgs.length);
    return this.apply(push).guess(options, [...preArgs, push], steps);
  }
...
class App extends Expr {
  ...
  guess (options = {}, preArgs = [], steps = 0) {
    ...
    const proxy = super.guess(options, preArgs, steps);
    if (proxy.grounded)
      return proxy;
    steps = proxy.steps; // reimport extra iterations

    const [fun, arg] = this.split().map(e => e.guess({ ...options, maxArgs: maxArgs / 2 }));
...
class Lambda extends Expr {
  ...
  guess (options = {}, preArgs = [], steps = 0) {
    ...
    const push = nthvar(preArgs.length);
    return this.reduce([push]).guess(options, [...preArgs, push], steps + 1);
  }
...
class Alias extends Named {
  ...
  guess (options = {}, preArgs = [], steps = 0) {
    return this.impl.guess(options, preArgs, steps);
  }
