...
class Expr {
  ...
  * lambdify (options = {}) {
    ...
    yield { expr, comment: 'term breakup' };
...
/**
 * @desc Generate a function that returns a new free variable each time it is called.
 ...
 */
/*
function freeSeq (options = {}) {
  let n = options.start ?? 0;
  const prefix = options.prefix ?? 'x';
  const html = options.html ?? false;
  const latinBefore = options.latin ?? 0;

  return function () {
    if (n < latinBefore && n < latin.length)
      return new FreeVar(latin[n++]);
    return new FreeVar( prefix + (html ? '<sub>' + ++n + '</sub>' : ++n) );
  }
}
*/
...
function * simplifyLambda (expr, options = {}, maxWeight = Infinity) {
  ...
  if (expr instanceof App) {
    ...
    for (const outer of simplifyLambda(arg, options, maxWeight - 1)) {
      for (const inner of simplifyLambda(fun, options, maxWeight - 1)) {
        const candidate = inner.expr.apply(outer.expr);
        if (candidate.weight() < maxWeight) {
          maxWeight = candidate.weight();
          yield { expr: candidate, steps: outer.steps + inner.steps, comment: inner.comment + '(fun)' };
        }
      }
    }
  }
  ...
  if (canon.canonical && canon.canonical.weight() < maxWeight) {
    ...
    yield { expr: canon.canonical, steps: canon.steps, comment: 'canonical' };
  }
}
