...
class App extends Expr {
  ...
  canonize (options = {}) {
    const maybe = super.canonize(options);
    if (maybe.canonical)
      return maybe;
    const fun = this.fun.canonize(options);
    maybe.steps += fun.steps;
    const arg = this.arg.canonize(options);
    maybe.steps += arg.steps;
    if (fun.canonical && arg.canonical)
      maybe.canonical = fun.canonical.apply(arg.canonical);
    return maybe;
  }
