...
class Expr {
  ...
  _guess (options, preArgs = [], steps = 0) {
    ...
    // if we're an App, the App's _guess will take care of further args
    if (this._firstVar())
      return { normal: false, steps };
  ...
  _aslist () {
    // currently only used by guess() but may be useful
  ...
  _firstVar () {
    ...
    // only used by guess() as a shortcut to this._aslist()[0] instanceof FreeVar
    return false;
  }
