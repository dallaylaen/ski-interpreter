...
class Expr {
  ...
  replace (terms, opt = {}) {
    ...
    for (const entry of terms) {
      ...
      pair[0] = pair[0].canonize(opt).canonical;
      if (!pair[0])
  ...
  _replace (pairs, opt) {
    const check = this.canonize(opt).canonical;
   ...
   *
   *       This is used internally when defining a Native term.
   *
   ...
   * }}
   */
  // TODO typedef returned value, rename canonical => expr, and document fields
  // TODO also rename canonize to something reflecting its heuristic nature
  canonize (options = {}, preArgs = [], steps = 0) {
    if (preArgs.length >= (options.maxArgs ?? globalOptions.maxArgs))
      return { grounded: false, steps };

    const pushVar = nthvar(preArgs.length);
    const next = this.run({ max: options.max ?? globalOptions.max }, pushVar);
    steps += next.steps;
    if (!next.final)
      return { grounded: false, steps };
    return next.expr.canonize(options, [...preArgs, pushVar], steps);
  }
...
class App extends Expr {
  ...
  canonize (options = {}, preArgs = [], steps = 0) {
    ...
    if (this.freeOnly()) {
      ...
      // pick the smallest expression from (1) super.canonize and (2) this.fun + this.arg
      // if none exists, bad luck

      const forward = this.wantsArgs()
        ? super.canonize(options, preArgs, steps)
        : { grounded: false, steps };
      if (forward.grounded)
        return forward; // we won't find any better than this
      steps = forward.steps; // reimport steps from super.canonize

      // console.log('[canonize] Found forward: ' + forward.canonical + ' with weight ' + forward.canonical?.weight() + ' for ' + preArgs + ' -> ' + this);

      // try to split as a last resort; limit arity in subexpressions
      const maxArgs = ((options.maxArgs ?? globalOptions.maxArgs) - preArgs.length) / 2;
      const [fun, arg] = this.split().map(
        e => e.canonize({ ...options, maxArgs }));
      expr = fun.canonical && arg.canonical && fun.canonical.apply(arg.canonical);
      steps += fun.steps + arg.steps;

      if (forward.canonical && (!expr || forward.canonical.weight() < expr.weight()))
        return { ...forward, steps };
      // otherwise, forward is discarded and we go with expr

      duplicate = duplicate || fun.duplicate || arg.duplicate;
      discard   = discard   || fun.discard   || arg.discard;
      grounded  = fun.grounded && arg.grounded && !fun.canonical.wantsArgs();
    }

    if (!expr)
      return { grounded: false, steps: 0 };

    const sym = expr.getSymbols();
    const [skip, dup] = skipDup(preArgs, sym);

    duplicate = duplicate || !!dup.size;
    discard   = discard   || !!skip.size;

    // console.log('found last application: ' + preArgs + ' -> ' + expr);

    return {
      grounded,
      steps,
      canonical: maybeLambda(preArgs, expr),
      proper:    isSubset(sym.keys(), new Set(preArgs)),
      linear:    !duplicate && !discard,
      arity:     preArgs.length,
      duplicate,
      discard,
      ...(skip.size ? { skip } : {}),
      ...(dup.size ? { dup } : {}),
...
class FreeVar extends Named {
  ...
  canonize (options = {}, preArgs = [], steps = 0) {
    ...
    return {
      grounded:  true,
      steps,
      canonical: maybeLambda(preArgs, this),
      proper,
      linear:    !skip.size,
      arity:     preArgs.length,
      duplicate: false,
...
class Native extends Named {
  ...
  constructor (name, impl, opt = {}) {
    ...
    const guess = (opt.canonize ?? true) ? this.canonize() : { grounded: false };
...
class Lambda extends Expr {
  ...
  canonize (options = {}, preArgs = [], steps = 0) {
    if (preArgs.length >= (options.maxArgs ?? globalOptions.maxArgs))
      return { grounded: false, steps };
    // cannot use this.arg directly as it may be duplicated and all hell breaks loose
    const nextVar = nthvar(preArgs.length);
    const next = this.impl.subst(this.arg, nextVar) ?? this.impl;
    const simplified = next.run( { max: options.max ?? globalOptions.max });
    steps += simplified.steps + 1; // one for sunstitution, too
    return (simplified.final ? simplified.expr : next)
      .canonize(options, [...preArgs, nextVar], steps);
  }
...
class Alias extends Named {
  ...
  canonize (options = {}, preArgs = [], steps = 0) {
    return this.expand().canonize(options, preArgs, steps);
  }
