...
class FreeVar extends Named {
   ...
   *
   * If context object is given, however, two variables with the same name and context
  ...
  diff (other, swap = false) {
    if (!(other instanceof FreeVar))
      return super.diff(other, swap);
    if (this.name === other.name && this.context === other.context)
      return null;
  ...
  subst (search, replace) {
    if (search instanceof FreeVar && search.name === this.name && search.context === this.context)
      return replace;
    return null;
  }
...
// it is required by rewriteSKI method, otherwise it could've as well be in parse.js
/**
