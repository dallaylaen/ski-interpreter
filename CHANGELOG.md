...
* don't copy web files into lib/ on module build (leftover from old build process)
* types were incorrectly stored in `lib/types/src` instead of just `lib/types`
...
* `./bin/ski.js quest-lint --fix` will fill in empty `"id":` and `"created_at":`
fields in the quest json file in question.
...
* remove build artifacts (docs/build/* and docs/man) from the repo.
* remove post-commit hook for good
...
* Make sure autogenerated quest timestamps are always in UTC
(caused build to fail after PR was submitted from a different timezone)

## [2.10.1] - 2026-07-10

### Fixed

* Remove `lib/types/test/**` from the package, nobody needs them.
* Clean up `lib/` before release.
